...
class JX3APP(Enum):
    """jx3api的接口枚举"""
    日常任务 = APP("/data/active/current", 0)
    开服检查 = APP("/data/server/check", 0)
    金价比例 = APP("/data/trade/demon", 0)
    沙盘图片 = APP("/app/sand", 0)
    家园鲜花 = APP("/data/home/flower", 0)
    科举答题 = APP("/data/exam/search", 0)
    家园家具 = APP("/data/home/furniture", 0)
    查器物谱 = APP("/data/home/travel", 0)
    推荐小药 = APP("/data/school/snacks", 0)
    推荐装备 = APP("/data/school/equip", 0)
    查宏命令 = APP("/data/school/macro", 0)
    官方资讯 = APP("/data/web/news", 0)
    维护公告 = APP("/data/web/announce", 0)
    阵眼效果 = APP("/data/school/matrix", 0)
    物品价格 = APP("/data/trade/search", 0)
    刷新地点 = APP("/data/index/search", 0)
    主从大区 = APP("/data/server/search", 0)
    随机骚话 = APP("/data/chat/random", 0)
    奇遇前置 = APP("/data/lucky/require", 0)
    奇遇攻略 = APP("/data/lucky/strategy", 0)
    奇遇查询 = APP("/data/lucky/serendipity", 10)
    奇遇统计 = APP("/data/lucky/statistical", 10)
    奇遇汇总 = APP("/data/lucky/collect", 10)
    资历排行 = APP("/data/school/seniority", 0)
    比赛战绩 = APP("/data/arena/recent", 10)
    战绩排行 = APP("/data/arena/awesome", 10)
    战绩统计 = APP("/data/arena/schools", 10)
    角色信息 = APP("/data/role/roleInfo", 0)
    装备属性 = APP("/data/role/attribute", 10)
    团队招募 = APP("/data/team/member/recruit", 10)
    烟花记录 = APP("/data/role/firework", 10)

    加密计算 = APP("/data/token/calculate", 0)
    查有效值 = APP("/data/token/ticket", 0)
    到期查询 = APP("/data/token/socket", 0)
