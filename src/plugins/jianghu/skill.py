...
class Skill():
    ...
    def 计算内力(self,
        ...
        有效变化 = 自己.内力变化(-消耗内力)
        self.战斗记录(
            f"<strong>{自己.名称}</strong> {技能名称} 内力<span class='text-primary'>{有效变化}</span>({自己.当前内力}/{自己.当前状态['内力上限']})"
        )
        return True

    def 治疗(self, 技能名称: str, 自己: UserInfo, 对方: UserInfo, 治疗量: int):
        有效变化 = 自己.气血变化(治疗量)
        self.触发被动("后手被动", "治疗", 有效变化, 自己, 对方)
        self.战斗记录(
            f"<strong>{自己.名称}</strong> < {技能名称}[治疗](+<span class='text-success'>{有效变化}</span>) 生命({自己.当前气血}/{自己.当前状态['气血上限']})"
        )
    ...
    def 造成伤害(self,
        ...
        if 防御 < 0:
            防御 = 0
        有效变化 = 守方.气血变化(-伤害数值 * (3000 / (3000 + 防御)))
        if 是否记录:
            攻方.本次伤害 += 有效变化
        self.战斗记录(
            f"<strong>{攻方.名称}</strong> {技能名称}[{伤害类型[:2]}](<span class='text-danger'>{有效变化}</span>) > <strong>{守方.名称}</strong>({守方.当前气血}/{守方.当前状态['气血上限']})"
        )
        return 守方.重伤状态, 有效变化
...
伤害结算完成后自己恢复 造成伤害 / 2 的内力值。
        ...
        有效变化 = 自己.内力变化(恢复内力)
        self.战斗记录(
            f"<strong>{自己.名称}</strong> 紫气东来 恢复内力 <span class='text-primary'>{有效变化}</span> ({自己.当前内力}/{自己.当前状态['内力上限']})"
        )
        return 重伤状态

    def 李代桃僵(self, 自己: UserInfo, 目标: UserInfo):
        """
李代桃僵
【触发类型】主动招式
【招式类型】内功招式
交换双方当前气血。
消耗内力 = 500 + 对方根骨 - 自己元气。最低消耗为 100 内力
        """
        消耗内力 = int(500 + 目标.基础属性["根骨"] - 自己.基础属性["元气"])
        if 消耗内力 < 100:
            消耗内力 = 100
        if self.计算内力("李代桃僵", 自己, 目标, 消耗内力):
            if 目标.基础属性.get("类型") == "首领":
                重伤状态, _ = self.造成伤害("李代桃僵", 自己, 目标, "内功伤害", 消耗内力 * 3)
                return 重伤状态
            气血差 = 自己.当前气血 - 目标.当前气血
            self.战斗记录(f"<strong>{自己.名称}</strong> 施放 李代桃僵")
            if 气血差 > 0:
                self.造成伤害("李代桃僵", 目标, 自己, "穿透伤害", 气血差)
                self.治疗("李代桃僵", 目标, 自己, 气血差)
            elif 气血差 < 0:
                self.造成伤害("李代桃僵", 自己, 目标, "穿透伤害", -气血差)
                self.治疗("李代桃僵", 自己, 目标, -气血差)
        return False
...
伤害 = (内功攻击 + 外功攻击) * (2.5 - |内功攻击 - 外功攻击| / (内功攻击 + 外功攻击)) * 2.5。
穿透 = (内功穿透 + 外功穿透) * 2。
内功攻击与内公共攻击总和越高，数值越接近，伤害越高。
最高为内功攻击与外功攻击总和的6.25倍，最低为内功攻击与外功攻击总和的2.5倍。
        """
        内功穿透 = 自己.当前状态["内功穿透"]
        外功穿透 = 自己.当前状态["外功穿透"]
        内功攻击 = 自己.当前状态["内功攻击"]
        外功攻击 = 自己.当前状态["外功攻击"]
        伤害数值 = int((内功攻击 + 外功攻击) * (2.5 - abs(内功攻击 - 外功攻击) / (内功攻击 + 外功攻击)) * 2.5)
...
穿透 = 对方外功穿透
    ...
    def 吐纳归元(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
吐纳归元
【触发类型】被动招式
【触发条件】受到治疗
...
增加元气 = 治疗量 * 35%
        ...
        增加元气 = int(数值 * 0.35)
        自己.改变当前状态({"元气": 增加元气})
        self.战斗记录(f"<strong>{自己.名称}</strong> 吐纳归元 增加元气 {增加元气}，当前元气 {自己.基础属性['元气']}")
        return 数值, False
...
【触发条件】外功攻击
【招式类型】外功招式
每进行一次普通攻击，自身外功攻击提高 4%
        """
        外功攻击 = 自己.当前状态["外功攻击"]
        外功攻击增加 = int(外功攻击 * 0.04)
...
触发概率 = 30%
    ...
    def 龙孙穿破(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
龙孙穿破
【触发类型】被动招式
【触发条件】外功攻击
...
恢复气血 = 即将造成伤害 * 35%
        """
        if randint(1, 100) < 17:
            防御 = 目标.当前状态["外功防御"] - 自己.当前状态["外功穿透"]
            if 防御 < 0:
                防御 = 0
            伤害 = int(数值 * (3000 / (3000 + 防御)))
            self.治疗("龙孙穿破", 自己, 目标, int(伤害 * 0.35))
        return 数值, False
...
触发概率：35%
        ...
        if randint(1, 100) < 35:
            ...
            return int(数值 * 1.7), False
        return 数值, False

    def 剑破万法(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
剑破万法
【触发类型】被动招式
【触发条件】外功攻击
...
触发概率：75%
    ...
    def 八宝金身(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
八宝金身
【触发类型】被动招式
【触发条件】受到内功、外功伤害
...
【触发条件】受到内功伤害
【招式类型】内功招式
吸取对方元气，吸取对方内力，降低受到的内功伤害，根据受到的伤害恢复内力。
吸取对方元气：对方元气*10%
降低伤害：自身元气*1.75
吸取内力：对方当前内力*20%
恢复内力：减伤的70%。
        """
        吸取内力 = 目标.当前内力 // 5
        减伤 = int(自己.基础属性["元气"] * 1.75)
        目标有效变化 = 目标.内力变化(-吸取内力)
        自己有效变化 = 自己.内力变化(吸取内力+int(减伤*0.7))
        改变元气 = 目标.基础属性["元气"] // 10
        数值 -= 减伤
        目标.改变当前状态({"元气": -改变元气})
        自己.改变当前状态({"元气": 改变元气})
...
【招式说明】
消耗自身气血对目标造成内功伤害。
当自己当前气血高于气血上限的25%时，自身减少当前气血的15%，对目标造成 当前气血 * 15% + 自己内功攻击 * 80% 的 内功伤害。
当自己当前气血低于气血上限的25%时，对目标造成 气血上限 * 10% + 自己内功攻击 * 80% 的 内功伤害。
        """
        if 自己.当前气血 >= 自己.当前状态["气血上限"] * 0.25:
            伤害 = int(自己.当前气血 * 0.15)
...
增加元气 = 根骨 * 1.5
        ...
        if not self.计算内力("推气过宫", 自己, 对方, 消耗内力):
            return ""
        self.治疗("推气过宫", 自己, 对方, int(自己.基础属性["元气"] * 3))
        自己.改变当前状态({"元气": int(自己.基础属性["根骨"] * 1.5)})
        return ""
...
【招式类型】外功招式
【招式说明】
消耗 20% 当前内力，造成对方气血上限 20% + 消耗内力值 的穿透伤害。
内力低于 50% 时无法施放。
        """
        ...
        if not self.计算内力("摧山坼地", 自己, 目标, 消耗内力, 需要内力):
            return ""
        伤害 = int(目标.当前状态["气血上限"] * 0.2) + 消耗内力 // 2
        if 目标.基础属性.get("类型") == "首领":
            伤害 = 自己.当前状态["外功攻击"] * 2 + 消耗内力
