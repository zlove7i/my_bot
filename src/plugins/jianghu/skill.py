import os
from datetime import datetime
from random import choice, randint
from src.plugins.jianghu.user_info import UserInfo
from src.utils.db import db
"""技能:
  - 主动技能
    - 主动攻击
    - 主动辅助
  - 被动技能
  - 状态加持"""


class 战斗计数():

    def __init__(self) -> None:
        self.伤害 = 0
        self.治疗 = 0


class Skill():

    def __init__(self) -> None:
        self.战斗内容 = []
        self.当前回合 = 1
        self.skill = {
            ""
            "势如破竹": {
                "type": "先手被动",
                "招式类型": "外功招式",
                "触发": ["外功伤害"],
                "招式": self.势如破竹
            },
            "雷霆万钧": {
                "type": "先手被动",
                "招式类型": "外功招式",
                "触发": ["外功伤害"],
                "招式": self.雷霆万钧
            },
            "龙孙穿破": {
                "type": "先手被动",
                "招式类型": "外功招式",
                "触发": ["外功伤害"],
                "招式": self.龙孙穿破
            },
            "剑破万法": {
                "type": "先手被动",
                "招式类型": "外功招式",
                "触发": ["外功伤害"],
                "招式": self.剑破万法
            },
            "天道剑势": {
                "type": "先手被动",
                "招式类型": "外功招式",
                "触发": ["外功伤害"],
                "招式": self.天道剑势
            },
            "三清剑法": {
                "type": "主动招式",
                "招式类型": "外功招式",
                "招式": self.三清剑法
            },
            "李代桃僵": {
                "type": "主动招式",
                "招式类型": "内功招式",
                "招式": self.李代桃僵
            },
            "两仪化形": {
                "type": "主动招式",
                "招式类型": "内功招式",
                "招式": self.两仪化形
            },
            "紫气东来": {
                "type": "主动招式",
                "招式类型": "内功招式",
                "招式": self.紫气东来
            },
            "吸星大法": {
                "type": "主动招式",
                "招式类型": "内功招式",
                "招式": self.吸星大法
            },
            "十方鬼道": {
                "type": "主动招式",
                "招式类型": "内功招式",
                "招式": self.十方鬼道
            },
            "推气过宫": {
                "type": "主动招式",
                "招式类型": "内功招式",
                "招式": self.推气过宫
            },
            "摧山坼地": {
                "type": "主动招式",
                "招式类型": "外功招式",
                "招式": self.摧山坼地
            },
            "剑若惊鸿": {
                "type": "主动招式",
                "招式类型": "外功招式",
                "招式": self.剑若惊鸿
            },
            "怖畏暗刑": {
                "type": "主动招式",
                "招式类型": "外功招式",
                "招式": self.怖畏暗刑
            },
            "剑心通明": {
                "type": "主动招式",
                "招式类型": "内功招式",
                "招式": self.剑心通明
            },
            "八宝金身": {
                "type": "后手被动",
                "招式类型": "内功招式",
                "触发": ["外功伤害", "内功伤害"],
                "招式": self.八宝金身
            },
            "北冥神功": {
                "type": "后手被动",
                "招式类型": "内功招式",
                "触发": ["内功伤害"],
                "招式": self.北冥神功
            },
            "斗转星移": {
                "type": "后手被动",
                "招式类型": "外功招式",
                "触发": ["外功伤害"],
                "招式": self.斗转星移
            },
            "吐纳归元": {
                "type": "后手被动",
                "招式类型": "内功招式",
                "触发": ["治疗"],
                "招式": self.吐纳归元
            }
        }

    def 战斗记录(self, 战斗记录):
        if len(self.战斗内容) < self.当前回合+1:
            self.战斗内容.append([])
        self.战斗内容[self.当前回合].append(战斗记录)

    def 触发被动(self, 被动类型: str, 事件: str, 数值: int, 自己: UserInfo, 目标: UserInfo):
        武学 = 自己.基础属性["武学"]
        if not any(武学):
            return 数值, ""
        for i in set(武学):
            招式 = self.skill.get(i, {})
            if 招式.get("type") == 被动类型 and 事件 in 招式.get("触发", []):
                数值, 重伤状态 = 招式["招式"](自己, 目标, 数值)
                if 重伤状态:
                    break
        return 数值, False

    def 计算内力(self,
             技能名称: str,
             自己: UserInfo,
             对方: UserInfo,
             消耗内力: int,
             需要内力: int = None,
             是否普通攻击: bool = True):
        if not 需要内力:
            需要内力 = 消耗内力
        有效变化 = 自己.内力变化(-需要内力)
        if 有效变化 == 0:
            if 是否普通攻击:
                self.造成伤害(f"{技能名称}(内力不足)",
                          自己, 对方, *自己.普通攻击())
            return False
        self.战斗记录(
            f"{自己.名称} {技能名称} 内力 {有效变化}({自己.当前内力}/{自己.当前状态['内力上限']})"
        )
        return True

    def 治疗(self, 技能名称: str, 自己: UserInfo, 对方: UserInfo, 治疗量: int):
        有效变化 = 自己.气血变化(治疗量)
        self.触发被动("后手被动", "治疗", 有效变化, 自己, 对方)
        self.战斗记录(
            f"{自己.名称} < {技能名称}[治疗](+{有效变化}) 生命({自己.当前气血}/{自己.当前状态['气血上限']})"
        )

    def 造成伤害(self,
             技能名称: str,
             攻方: UserInfo,
             守方: UserInfo,
             伤害类型: str,
             伤害数值: float,
             穿透: float = 0,
             是否记录: bool = True) -> bool:
        """造成伤害"""
        # 计算先手被动
        伤害数值, 重伤信息 = self.触发被动("先手被动", 伤害类型, 伤害数值, 攻方, 守方)
        if 重伤信息:
            return 重伤信息, 伤害数值
        # 计算后手被动
        伤害数值, 重伤信息 = self.触发被动("后手被动", 伤害类型, 伤害数值, 守方, 攻方)
        if 重伤信息:
            return 重伤信息, 伤害数值
        if 伤害类型 == "外功伤害":
            防御 = 守方.当前状态["外功防御"] - 穿透
        elif 伤害类型 == "内功伤害":
            防御 = 守方.当前状态["内功防御"] - 穿透
        elif 伤害类型 == "混合伤害":
            防御 = 守方.当前状态["外功防御"] + 守方.当前状态["内功防御"] - 穿透
        elif 伤害类型 == "穿透伤害":
            防御 = 0
        if 防御 < 0:
            防御 = 0
        有效变化 = 守方.气血变化(-伤害数值 * (3000 / (3000 + 防御)))
        if 是否记录:
            攻方.本次伤害 += 有效变化
        self.战斗记录(
            f"{攻方.名称} {技能名称}[{伤害类型[:2]}]({有效变化}) > {守方.名称}({守方.当前气血}/{守方.当前状态['气血上限']})"
        )
        return 守方.重伤状态, 有效变化

    def 三清剑法(self, 自己: UserInfo, 目标: UserInfo):
        """
三清剑法
【触发类型】主动招式
【招式类型】外功招式
根据自身速度与对方速度差进行多次普通攻击。
攻击次数：(自身速度 - 对方速度) ^ 0.25 / 2 取整) + 1。
伤害：普通攻击 * 伤害系数
伤害系数：0.6
穿透：自身外功穿透
        """
        速度差 = 自己.当前状态['速度'] - 目标.当前状态['速度']
        if 速度差 < 0:
            速度差 = 0
        攻击次数 = int((速度差 ** 0.25) / 2 + 1)
        伤害系数 = 0.6
        self.战斗记录("施放三清剑法")
        for _ in range(攻击次数):
            伤害类型, 伤害数值, 穿透 = 自己.普通攻击()
            重伤状态, _ = self.造成伤害("普通攻击", 自己, 目标, 伤害类型, 伤害数值 * 伤害系数, 穿透)
            if 重伤状态:
                break
        return 重伤状态

    def 两仪化形(self, 自己: UserInfo, 目标: UserInfo):
        """
两仪化形
【触发类型】主动招式
【招式类型】内功招式
根据自身内攻攻击对敌方造成随机伤害。并对减少目标当前速度。
伤害 = 内攻攻击 * (1 ~ 8随机数)
穿透 = 自身内功穿透
减速 = 目标当前速度 * 0.13
内力消耗 = 100
        """
        内功穿透 = 自己.当前状态["内功穿透"]
        伤害数值 = 自己.当前状态["内功攻击"] * randint(1, 8)
        消耗内力 = 100
        if not self.计算内力("两仪化形", 自己, 目标, 消耗内力):
            return ""
        减速 = int(目标.当前状态['速度'] * 0.13)
        self.战斗记录(
            f"{目标.名称} 速度-{减速}"
        )
        目标.改变当前状态({"速度": -减速})
        重伤状态, _ = self.造成伤害("两仪化形", 自己, 目标, "内功伤害", 伤害数值, 内功穿透)
        return 重伤状态

    def 紫气东来(self, 自己: UserInfo, 目标: UserInfo):
        """
紫气东来
【触发类型】主动招式
【招式类型】内功招式
造成内攻伤害的同时恢复内力值。
消耗当前 元气 * 0.3 的内力。对敌方造成 内功攻击 * 1.7 的内功伤害。
若当前内力低于 元气 * 0.3，则对敌方造成 内功攻击 * 0.7 的内功伤害。
伤害结算完成后自己恢复 造成伤害 / 2 的内力值。
        """
        内功穿透 = 自己.当前状态["内功穿透"]
        消耗内力 = int(自己.基础属性["元气"] * 0.3)
        if self.计算内力("紫气东来", 自己, 目标, 消耗内力, 是否普通攻击=False):
            伤害数值 = int(自己.当前状态["内功攻击"] * 1.7)
        else:
            伤害数值 = int(自己.当前状态["内功攻击"] * 0.7)
        重伤状态, 实际伤害 = self.造成伤害("紫气东来", 自己, 目标, "内功伤害", 伤害数值, 内功穿透)
        恢复内力 = -实际伤害 // 2
        if 恢复内力 > (自己.当前状态["内力上限"] - 自己.当前内力):
            恢复内力 = 自己.当前状态["内力上限"] - 自己.当前内力
        有效变化 = 自己.内力变化(恢复内力)
        self.战斗记录(
            f"{自己.名称} 紫气东来 恢复内力 {有效变化} ({自己.当前内力}/{自己.当前状态['内力上限']})"
        )
        return 重伤状态

    def 李代桃僵(self, 自己: UserInfo, 目标: UserInfo):
        """
李代桃僵
【触发类型】主动招式
【招式类型】内功招式
交换双方当前气血。
消耗内力 = 500 + 对方根骨 - 自己元气。最低消耗为 100 内力
        """
        消耗内力 = int(500 + 目标.基础属性["根骨"] - 自己.基础属性["元气"])
        if 消耗内力 < 100:
            消耗内力 = 100
        if self.计算内力("李代桃僵", 自己, 目标, 消耗内力):
            if 目标.基础属性.get("类型") == "首领":
                重伤状态, _ = self.造成伤害("李代桃僵", 自己, 目标, "内功伤害", 消耗内力 * 3)
                return 重伤状态
            气血差 = 自己.当前气血 - 目标.当前气血
            self.战斗记录(f"<strong>{自己.名称}</strong> 施放 李代桃僵")
            if 气血差 > 0:
                self.造成伤害("李代桃僵", 目标, 自己, "穿透伤害", 气血差)
                self.治疗("李代桃僵", 目标, 自己, 气血差)
            elif 气血差 < 0:
                self.造成伤害("李代桃僵", 自己, 目标, "穿透伤害", -气血差)
                self.治疗("李代桃僵", 自己, 目标, -气血差)
        return False

    def 吸星大法(self, 自己: UserInfo, 目标: UserInfo):
        """
吸星大法
【触发类型】主动招式
【招式类型】内功招式
根据敌方当前气血与自己放内功攻击对敌方造成内功伤害, 并根据此伤害恢复气血
消耗当前 元气 * 0.3 的内力。对敌方造成 内功攻击 * 2.5 + 目标当前气血 * 0.05 的内功伤害
伤害结算完成后自己恢复 造成伤害 / 1.5 的气血值。
        """
        内功穿透 = 自己.当前状态["内功穿透"]
        消耗内力 = int(自己.基础属性["元气"] * 0.3)
        if not self.计算内力("吸星大法", 自己, 目标, 消耗内力):
            return
        if 目标.基础属性.get("类型") == "首领":
            伤害数值 = int(自己.当前状态["内功攻击"] * 3.75)
        else:
            伤害数值 = int(自己.当前状态["内功攻击"] * 2.5 + 目标.当前气血 * 0.05)

        重伤状态, 实际伤害 = self.造成伤害("吸星大法", 自己, 目标, "内功伤害", 伤害数值, 内功穿透)
        恢复气血 = -实际伤害 // 1.5
        self.治疗("吸星大法", 自己, 目标, 恢复气血)
        return 重伤状态

    def 剑若惊鸿(self, 自己: UserInfo, 目标: UserInfo):
        """
剑若惊鸿
【触发类型】主动招式
【招式类型】外功招式
根据自身的内功攻击与外功攻击对敌方造成混合伤害。
伤害 = (内功攻击 + 外功攻击) * (2.5 - |内功攻击 - 外功攻击| / (内功攻击 + 外功攻击)) * 2.5。
穿透 = (内功穿透 + 外功穿透) * 2。
内功攻击与内公共攻击总和越高，数值越接近，伤害越高。
最高为内功攻击与外功攻击总和的6.25倍，最低为内功攻击与外功攻击总和的2.5倍。
        """
        内功穿透 = 自己.当前状态["内功穿透"]
        外功穿透 = 自己.当前状态["外功穿透"]
        内功攻击 = 自己.当前状态["内功攻击"]
        外功攻击 = 自己.当前状态["外功攻击"]
        伤害数值 = int((内功攻击 + 外功攻击) * (2.5 - abs(内功攻击 - 外功攻击) / (内功攻击 + 外功攻击)) * 2.5)
        重伤状态, _ = self.造成伤害("剑若惊鸿", 自己, 目标, "混合伤害", 伤害数值, (内功穿透+外功穿透) * 2)
        return 重伤状态

    def 斗转星移(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
斗转星移
【触发类型】被动招式
【触发条件】受到外功伤害
【招式类型】外功招式
遭受攻击时将受到的伤害返还给对方。
触发概率 = 25%
反击伤害 = 对方普通攻击伤害 * 100% 的混合伤害。
穿透 = 对方外功穿透
        """
        if randint(1, 100) < 25:
            _, 伤害数值, 穿透 = 目标.普通攻击()
            重伤状态, _ = self.造成伤害("斗转星移", 自己, 目标, "混合伤害", 伤害数值, 穿透)
            return 数值, 重伤状态
        return 数值, False

    def 吐纳归元(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
吐纳归元
【触发类型】被动招式
【触发条件】受到治疗
【招式类型】内功招式
受到治疗时增加元气。
增加元气 = 治疗量 * 35%
        """
        增加元气 = int(数值 * 0.35)
        自己.改变当前状态({"元气": 增加元气})
        self.战斗记录(f"{自己.名称} 吐纳归元 增加元气 {增加元气}，当前元气 {自己.基础属性['元气']}")
        return 数值, False

    def 天道剑势(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
天道剑势
【触发类型】被动招式
【触发条件】外功攻击
【招式类型】外功招式
每进行一次普通攻击，自身外功攻击提高 4%
        """
        外功攻击 = 自己.当前状态["外功攻击"]
        外功攻击增加 = int(外功攻击 * 0.04)
        自己.改变当前状态({"外功攻击": 外功攻击增加})
        self.战斗记录(
            f"{自己.名称} 天道剑势 外功攻击 {外功攻击} +{外功攻击增加}"
        )
        return 数值, False

    def 势如破竹(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
势如破竹
【触发类型】被动招式
【触发条件】外功攻击
【招式类型】外功招式
对方外功防御减少 6%
触发概率 = 30%
        """
        if randint(1, 100) < 30:
            目标外防 = 目标.当前状态["外功防御"]
            外防减少 = int(目标外防 * 0.06)
            目标.改变当前状态({"外功防御": -外防减少})
            self.战斗记录(f"{自己.名称} 势如破竹 {目标.名称}外功防御-{外防减少}")
            return 数值, False
        return 数值, False

    def 龙孙穿破(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
龙孙穿破
【触发类型】被动招式
【触发条件】外功攻击
【招式类型】外功招式
根据即将造成的外功伤害恢复气血
触发概率 = 17%
恢复气血 = 即将造成伤害 * 35%
        """
        if randint(1, 100) < 17:
            防御 = 目标.当前状态["外功防御"] - 自己.当前状态["外功穿透"]
            if 防御 < 0:
                防御 = 0
            伤害 = int(数值 * (3000 / (3000 + 防御)))
            self.治疗("龙孙穿破", 自己, 目标, int(伤害 * 0.35))
        return 数值, False

    def 雷霆万钧(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
雷霆万钧
【触发类型】被动招式
【触发条件】外功攻击
【招式类型】外功招式
增加单次外功伤害
增加伤害：70%
触发概率：35%
        """
        if randint(1, 100) < 35:
            self.战斗记录(f"{自己.名称} 雷霆万钧")
            return int(数值 * 1.7), False
        return 数值, False

    def 剑破万法(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
剑破万法
【触发类型】被动招式
【触发条件】外功攻击
【招式类型】外功招式
造成伤害时削减目标内力
削减内力：本次攻击伤害 * 30%
触发概率：75%
        """
        if randint(1, 100) < 75:
            削减内力 = 数值 * 0.3
            有效变化 = 目标.内力变化(-削减内力, True)
            self.战斗记录(f"{自己.名称} 剑破万法 {目标.名称} 内力{有效变化}")
            return 数值, False
        return 数值, False

    def 八宝金身(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
八宝金身
【触发类型】被动招式
【触发条件】受到内功、外功伤害
【招式类型】内功招式
消耗内力抵消伤害部分。
抵消伤害比例：80%
每点内力抵消伤害：8
触发概率：80%
        """
        if randint(1, 100) > 80:
            return 数值, False
        消耗内力 = (数值 * 0.8) / 8
        有效变化 = 自己.内力变化(-消耗内力)
        减伤 = 有效变化 * 8
        if 减伤 != 0:
            self.战斗记录(f"{自己.名称} 八宝金身 内力 {有效变化} 减伤 {减伤}")
        数值 += 减伤
        return 数值, False

    def 北冥神功(self, 自己: UserInfo, 目标: UserInfo, 数值: int):
        """
北冥神功
【触发类型】被动招式
【触发条件】受到内功伤害
【招式类型】内功招式
吸取对方元气，吸取对方内力，降低受到的内功伤害，根据受到的伤害恢复内力。
吸取对方元气：对方元气*10%
降低伤害：自身元气*1.75
吸取内力：对方当前内力*20%
恢复内力：减伤的70%。
        """
        吸取内力 = 目标.当前内力 // 5
        减伤 = int(自己.基础属性["元气"] * 1.75)
        目标有效变化 = 目标.内力变化(-吸取内力)
        自己有效变化 = 自己.内力变化(吸取内力+int(减伤*0.7))
        改变元气 = 目标.基础属性["元气"] // 10
        数值 -= 减伤
        目标.改变当前状态({"元气": -改变元气})
        自己.改变当前状态({"元气": 改变元气})
        self.战斗记录(f"{自己.名称} 北冥神功 吸取内力 {自己有效变化} ({自己.当前内力}/{自己.当前状态['内力上限']}) {目标.名称} 内力 {目标有效变化} 元气{改变元气}")
        return 数值, False

    def 十方鬼道(self, 自己: UserInfo, 目标: UserInfo):
        """
十方鬼道
【触发类型】主动招式
【招式类型】内功招式
【招式说明】
消耗自身气血对目标造成内功伤害。
当自己当前气血高于气血上限的20%时，自身减少当前气血的15%，对目标造成 当前气血 * 15% + 自己内功攻击 * 50% 的 内功伤害。
当自己当前气血低于气血上限的20%时，对目标造成 气血上限 * 10% + 自己内功攻击 * 50% 的 内功伤害。
        """
        if 自己.当前气血 >= 自己.当前状态["气血上限"] * 0.2:
            伤害 = int(自己.当前气血 * 0.15)
            self.造成伤害("十方鬼道(消耗)", 自己, 自己, "内功伤害", 伤害, 是否记录=False)
        else:
            伤害 = int(自己.当前状态["气血上限"] * 0.1)
        重伤状态, _ = self.造成伤害("十方鬼道", 自己, 目标, "内功伤害",
                            伤害 + int(自己.当前状态["内功攻击"] * 0.8), 自己.当前状态["内功穿透"])
        return 重伤状态

    def 推气过宫(self, 自己: UserInfo, 对方: UserInfo):
        """
推气过宫
【触发类型】主动招式
【招式类型】内功招式
【招式说明】
为自己回复当前气血，并提高自身元气。
内力消耗 = 根骨 * 0.4
恢复气血 = 恢复自己 元气 * 3
增加元气 = 根骨 * 1.5
        """
        消耗内力 = int(自己.基础属性["根骨"] * 0.4)
        if not self.计算内力("推气过宫", 自己, 对方, 消耗内力):
            return ""
        self.治疗("推气过宫", 自己, 对方, int(自己.基础属性["元气"] * 3))
        自己.改变当前状态({"元气": int(自己.基础属性["根骨"] * 1.5)})
        return ""

    def 摧山坼地(self, 自己: UserInfo, 目标: UserInfo):
        """
摧山坼地
【触发类型】主动招式
【招式类型】外功招式
【招式说明】
消耗 20% 当前内力，造成对方气血上限 17% + 消耗内力值 的穿透伤害。
内力低于 50% 时无法施放。
        """
        消耗内力 = int(自己.当前内力 * 0.2)
        需要内力 = 自己.当前状态["内力上限"] * 0.5
        if not self.计算内力("摧山坼地", 自己, 目标, 消耗内力, 需要内力):
            return ""
        伤害 = int(目标.当前状态["气血上限"] * 0.17) + 消耗内力 // 2
        if 目标.基础属性.get("类型") == "首领":
            伤害 = 自己.当前状态["外功攻击"] * 1.7 + 消耗内力
        重伤状态, _ = self.造成伤害("摧山坼地", 自己, 目标, "穿透伤害", 伤害)
        return 重伤状态

    def 怖畏暗刑(self, 自己: UserInfo, 目标: UserInfo):
        """
怖畏暗刑
【触发类型】主动招式
【招式类型】外功招式
【招式说明】
对目标施展缴械并附带少量外功伤害。
消耗内力：100
成功率：100%-((100 + 目标状态抗性)/(200 + 己方速度))%
缴械回合：3 - 目标状态抗性 /700取整，若小于1则生效1回合
伤害：己方外功攻击 * 1
        """
        if not self.计算内力("怖畏暗刑", 自己, 目标, 100):
            return
        目标状态抗性 = 目标.当前状态["状态抗性"]
        己方速度 = 自己.当前状态["速度"]
        if randint(1, 200 + 己方速度) < 目标状态抗性 + 100:
            self.战斗记录("怖畏暗刑施放失败")
            return
        有效回合 = 3 - 目标状态抗性 // 700
        if 有效回合 < 1:
            有效回合 = 1
        self.战斗记录(f"怖畏暗刑施放成功，{目标.名称}缴械{有效回合}回合")
        目标.debuff.append({"名称": "怖畏暗刑", "type": "缴械", "剩余回合": 有效回合})
        伤害 = 自己.当前状态["外功攻击"]
        重伤状态, _ = self.造成伤害("怖畏暗刑", 自己, 目标, "外功伤害", 伤害, 自己.当前状态["外功穿透"])
        return 重伤状态

    def 剑心通明(self, 自己: UserInfo, 目标: UserInfo):
        """
剑心通明（临时）
【触发类型】主动招式
【招式类型】内功招式
【招式说明】
对目标施展封内并附带少量内功伤害。
消耗内力：100
成功率：100%-((100 + 目标状态抗性)/(200 + 己方速度))%
缴械回合：3 - 目标状态抗性 /700取整，若小于1则生效1回合
伤害：己方内功攻击 * 1
        """
        if not self.计算内力("剑心通明", 自己, 目标, 100):
            return
        目标状态抗性 = 目标.当前状态["状态抗性"]
        己方速度 = 自己.当前状态["速度"]
        if randint(1, 200 + 己方速度) < 目标状态抗性 + 100:
            self.战斗记录("剑心通明施放失败")
            return
        有效回合 = 3 - 目标状态抗性 // 700
        if 有效回合 < 1:
            有效回合 = 1
        self.战斗记录(f"剑心通明施放成功，{目标.名称}封内{有效回合}回合")
        目标.debuff.append({"名称": "剑心通明", "type": "封内", "剩余回合": 有效回合})
        伤害 = 自己.当前状态["内功攻击"]
        重伤状态, _ = self.造成伤害("剑心通明", 自己, 目标, "内功伤害", 伤害, 自己.当前状态["内功穿透"])
        return 重伤状态

